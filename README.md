...
- **Changes planned for the next release:** The following are the main changes planned for the next release. Please note that these changes may be subject to change without notice before the release.
    ...
    - Added support for `transformers` 5.6 and later. [PR #2437](https://github.com/kohya-ss/sd-scripts/pull/2437)
        ...
        - `transformers` 5.6 also switched the attention implementation of T5 (used by FLUX.1 / SD3 as T5-XXL, and by HunyuanImage as byT5) to SDPA, which changes the bf16/fp16 outputs slightly. sd-scripts keeps the previous (eager) implementation so that the text encoder outputs, and therefore the cached text encoder outputs, remain identical across versions.
        - The pinned version in `requirements.txt` is unchanged (5.5.4). Text encoder outputs of all supported models were verified to be identical with `transformers` 5.17.0 using the local regression tests in `tests/local`.
