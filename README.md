...
| `--grad_norm_mode {stable,stable_no_threshoff,gamble}` | `None` | `skip_grad_norm` 系の推奨プリセットを一括指定。`stable`=安定重視、`stable_no_threshoff`=`stable` から ThreshOff 区間を抑制、`gamble`=当たり狙い。 | 指定時は個別オプションを無視し、`--no-skip_nan_immediate` / `--no-skip_inf_immediate` のみ上書き可。詳細: [docs/skip_grad_norm_refactor_plan-ja.md](docs/skip_grad_norm_refactor_plan-ja.md) |
