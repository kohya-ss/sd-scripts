...
def train(args):
    ...
    orig_embeds_params = (
        unwrap_model(text_encoder).get_input_embeddings().weight.data.detach().clone()
    )
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(text_encoder):
                ...
                encoder_hidden_states = train_util.get_hidden_states(
                    args, input_ids, tokenizer, text_encoder, torch.float
                )
                ...
                with torch.no_grad():
                    unwrap_model(text_encoder).get_input_embeddings().weight[
                        index_no_updates
            ...
            if args.logging_dir is not None:
                ...
                if (
                    args.optimizer_type.lower() == "DAdaptation".lower()
                ):  # tracking d*lr value
                    logs["lr/d*lr"] = (
        ...
        if args.save_every_n_epochs is not None:
            ...
            def save_func():
                ckpt_name = (
                    train_util.EPOCH_FILE_NAME.format(model_name, epoch + 1)
                    + "."
                    + args.save_model_as
                )
            ...
            def remove_old_func(old_epoch_no):
                old_ckpt_name = (
                    train_util.EPOCH_FILE_NAME.format(model_name, old_epoch_no)
                    + "."
                    + args.save_model_as
                )
    ...
    if is_main_process:
        ...
        model_name = (
            train_util.DEFAULT_LAST_OUTPUT_NAME
            if args.output_name is None
            else args.output_name
        )
