...
`--dq_delta_auto_preset clip_rate_low_auto` では、最初は `clip_rate_low` と同じ低clip帯を狙う。学習中に `QuantErrRatioEMA >= 0.25` かつ `QErrPerClip >= 130` が連続し、lowを維持するコストが高いと判断した場合は、目標clip帯を `clip_rate_mid` へ一方向に切り替える。これは低clipが合うデータではlowを維持し、低clipが合わないデータでは誤差が深くなる前に中間帯へ逃がすための保険である。既定では学習進捗 0.55 で切替を凍結する。よく使う `--avg_begin 0.6` より前に目標clip帯を確定させ、`avg_cp` の対象区間で目標変更に伴う `range_mul` の急変が起きにくいようにするためである。
