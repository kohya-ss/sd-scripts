...
def prepare_dataset_args(args: argparse.Namespace, support_metadata: bool):
    ...
    if args.cache_latents:
        assert (
            not args.color_aug
        ), "when caching latents, color_aug cannot be used / latentをキャッシュするときはcolor_augは使えません"
        assert (
            not args.random_crop
        ), "when caching latents, random_crop cannot be used / latentをキャッシュするときはrandom_cropは使えません"
...
def load_target_model(args: argparse.Namespace, weight_dtype):
    ...
    if load_stable_diffusion_format:
        ...
        text_encoder, vae, unet = load_models_from_stable_diffusion_checkpoint(
            args.v2, name_or_path
        )
    else:
        ...
        try:
            ...
            raise KohyaException(
                f"Model either doesn't exist as a file or on HuggingFace.co. Invalid filename? / "
...
def get_epoch_ckpt_name(args: argparse.Namespace, use_safetensors, epoch):
    ...
    ckpt_name = EPOCH_FILE_NAME.format(model_name, epoch) + (
...
def save_on_epoch_end(
    ...
    if saving:
        ...
        if args.save_last_n_epochs is not None:
            remove_epoch_no = (
                epoch_no - args.save_every_n_epochs * args.save_last_n_epochs
            )
...
def save_sd_model_on_epoch_end(
    ...
    if save_stable_diffusion_format:
        ...
        def save_du():
            out_dir: pathlib.Path = args.output_dir.joinpath(
                EPOCH_DIFFUSERS_DIR_NAME.format(model_name, epoch_no)
            )
    ...
    saving = save_on_epoch_end(
        args, save_func, remove_old_func, epoch_no, num_train_epochs
    )
    if saving and args.save_state:
        save_state_on_epoch_end(args, accelerator, model_name, epoch_no)


def save_state_on_epoch_end(
    args: argparse.Namespace, accelerator, model_name, epoch_no
):
    print("saving state.")
    ...
    if last_n_epochs is not None:
        ...
        state_dir_old = args.output_dir / EPOCH_STATE_NAME.format(
            model_name, remove_epoch_no
        )
...
def save_sd_model_on_train_end(
    ...
    model_name = (
        DEFAULT_LAST_OUTPUT_NAME if args.output_name is None else args.output_name
    )
