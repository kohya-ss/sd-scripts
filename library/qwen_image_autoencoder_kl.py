...
class QwenImageUpBlock2D(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, num_res_blocks: int, dropout: float = 0.0, upsample_mode: Optional[str] = None):
        super().__init__()
        resnets = []
        current_dim = in_dim
        for _ in range(num_res_blocks + 1):
...
class QwenImageDecoder2D(nn.Module):
    def __init__(
        ...
        for index, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            ...
            self.up_blocks.append(QwenImageUpBlock2D(in_dim, out_dim, num_res_blocks, dropout, upsample_mode))
...
class AutoencoderKLQwenImage2D(nn.Module):
    ...
    def encode(self, x: torch.Tensor, return_dict: bool = True) -> Union[Dict[str, torch.Tensor], Tuple[DiagonalGaussianDistribution]]:
        h = self._encode(x)
...
def is_qwen2d_state_dict(sd: Dict[str, torch.Tensor]) -> bool:
...
def load_vae(
    ...
    spatial_chunk_size: Optional[int] = None,
    disable_cache: bool = False,
) -> Union[AutoencoderKLQwenImage, AutoencoderKLQwenImage2D]:
...
"""
    ...
    vae_class = AutoencoderKLQwenImage2D if is_qwen2d_state_dict(state_dict) else AutoencoderKLQwenImage
    if vae_class is AutoencoderKLQwenImage2D:
        logger.info("Detected Qwen 2D VAE checkpoint")
        vae = vae_class(
