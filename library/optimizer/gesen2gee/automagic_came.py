...
class Automagic_Came(torch.optim.Optimizer):
    def __init__(
        self,
        params,
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                pre = state['pre'] if state['pre'] is not None else torch.zeros_like(p_data_fp32)
                condition = - torch.sum(torch.mul(grad, p_data_fp32 - pre))
                if condition < 0.0:
                    ratio = self._ratio(new_p, p_data_fp32, pre)
