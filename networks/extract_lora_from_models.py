...
import lora
from library.loaders import load_models_from_stable_diffusion_checkpoint
...
def svd(args):
    ...
    text_encoder_o, _, unet_o = load_models_from_stable_diffusion_checkpoint(
        args.v2, args.model_org
    )
    print(f"loading SD model : {args.model_tuned}")
    text_encoder_t, _, unet_t = load_models_from_stable_diffusion_checkpoint(
        args.v2, args.model_tuned
    )

    # create LoRA network to extract weights: Use dim (rank) as alpha
    lora_network_o = lora.create_network(
        1.0, args.dim, args.dim, None, text_encoder_o, unet_o
    )
    lora_network_t = lora.create_network(
        1.0, args.dim, args.dim, None, text_encoder_t, unet_t
    )
    ...
    lora_network_o.apply_to(
        text_encoder_o, unet_o, text_encoder_different, True
    )  # to make state dict
