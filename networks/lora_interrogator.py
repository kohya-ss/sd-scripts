...
def interrogate(args):
    ...
    text_encoder, vae, unet = load_models_from_stable_diffusion_checkpoint(args.v2, args.sd_model)
    ...
    if not has_te_weight:
        print("This LoRA does not have modules for Text Encoder, cannot interrogate / このLoRAはText "
              "Encoder向けのモジュールがないため調査できません")
        return
    del vae
    ...
    if args.v2:
        ...
        tokenizer: CLIPTokenizer = CLIPTokenizer.from_pretrained \
            (TOKENIZER_PATH)  # , model_max_length=max_token_length + 2)
