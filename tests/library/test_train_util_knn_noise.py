...
def test_dit_training_scripts_use_shared_training_noise_helper():
    ...
    for script_name in (
        ...
        assert "noise = train_util.sample_training_noise(args, latents)" in source
        assert "noise = torch.randn_like(latents)" not in source
