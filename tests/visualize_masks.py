...
showing both the raw mask and the mask applied to a test image.

Usage:
    python3 tests/visualize_masks.py [--out-dir tests/mask_viz] [--size 512] [--cols 4]
...
def _test_image(width: int, height: int, seed: int) -> Image.Image:
    """Reuse the same synthetic image generator as the training data."""
...
def make_grid(
    ...
    for i, (label, mask, source) in enumerate(samples):
        ...
        # resize everything to cell_size
...
def gallery_cloud(size: int, n: int, cols: int) -> Image.Image:
    samples = []
    param_sets = [
    ...
    for i in range(n):
        ...
        src  = _test_image(size, size, seed=i)
        lbl  = f"sc={p['base_scale']} oct={p['octaves']} per={p['persistence']} thr={p['threshold']}"
        samples.append((lbl, mask, src))
    return make_grid(samples, cols, size)


def gallery_polygon(size: int, n: int, cols: int) -> Image.Image:
    samples = []
    param_sets = [
...
def gallery_shape(size: int, n: int, cols: int) -> Image.Image:
    samples = []
    for i in range(n):
        ...
        mask = shape_mask(size, size, min_coverage=0.05, max_coverage=0.6,
                          shape=s, seed=i * 17)
        src  = _test_image(size, size, seed=i)
        samples.append((f"shape={s}", mask, src))
    return make_grid(samples, cols, size)


def gallery_combined(size: int, n: int, cols: int) -> Image.Image:
    samples = []
    for i in range(n):
        cloud = cloud_mask(size, size, octaves=6, persistence=0.5,
                           threshold=0.5, seed=i * 3)
        poly  = polygon_mask(size, size, n_points=6, irregularity=0.4,
                             min_coverage=0.1, max_coverage=0.5, seed=i * 5)
        mask  = combine_masks(cloud, poly)
        src   = _test_image(size, size, seed=i)
        samples.append((f"cloud+polygon #{i}", mask, src))
    return make_grid(samples, cols, size)


def gallery_random(size: int, n: int, cols: int) -> Image.Image:
    samples = []
    for i in range(n):
        mask = random_mask(size, size, seed=i * 11)
        src  = _test_image(size, size, seed=i)
        samples.append((f"random #{i}", mask, src))
    return make_grid(samples, cols, size)
...
def main():
    ...
    parser.add_argument("--size", type=int, default=256, help="Image size (pixels)")
    ...
    for filename, fn in galleries:
        ...
        img = fn(args.size, args.n, args.cols)
        img.save(out_path)
