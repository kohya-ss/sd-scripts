...
class Hypernetwork(torch.nn.Module):
    ...
    def __init__(self, multiplier=1.0) -> None:
        ...
        for size in Hypernetwork.enable_sizes:
            self.modules.append(
                (
                    HypernetworkModule(size, multiplier),
                    HypernetworkModule(size, multiplier),
                )
            )
    ...
    def load_from_state_dict(self, state_dict):
        ...
        for size, sd in state_dict.items():
            if type(size) == int:
                self.modules[Hypernetwork.enable_sizes.index(size)][0].load_state_dict(
                    sd[0], strict=True
                )
                self.modules[Hypernetwork.enable_sizes.index(size)][1].load_state_dict(
                    sd[1], strict=True
                )
        return True
