    ...
    def process_batch(is_last):
        for bucket in bucket_manager.buckets:
            if (is_last and len(bucket) > 0) or len(bucket) >= args.batch_size:
                ...
                assert (
                    latents.shape[2] == bucket[0][1].shape[0] // 8
                    and latents.shape[3] == bucket[0][1].shape[1] // 8
                ...
                for (image_key, _), latent in zip(bucket, latents):
                    npz_file_name = get_npz_filename_wo_ext(
                        args.train_data_dir, image_key, args.full_path, False
                    )
                ...
                if args.flip_aug:
                    latents = get_latents(
                        vae, [img[:, ::-1].copy() for _, img in bucket], weight_dtype
                    )  # copyがないとTensor変換できない

                    for (image_key, _), latent in zip(bucket, latents):
                        npz_file_name = get_npz_filename_wo_ext(
                            args.train_data_dir, image_key, args.full_path, True
                        )
                        np.savez(npz_file_name, latent)
                else:
                    ...
                    for image_key, _ in bucket:
                        ...
                        if os.path.isfile(npz_file_name):
                            print(
                                f"remove existing flipped npz / 既存のflipされたnpzファイルを削除します: {npz_file_name}"
                            )
    ...
    for data_entry in tqdm(data, smoothing=0.0):
        ...
        if args.skip_existing:
            ...
            for npz_file in npz_files:
                ...
                if (
                    dat.shape[1] != reso[1] // 8 or dat.shape[2] != reso[0] // 8
                ):  # latentsのshapeを確認
                    found = False
                    break
            if found:
        ...
        if (
            resized_size[0] != image.shape[1] or resized_size[1] != image.shape[0]
        ):  # リサイズ処理が必要？
