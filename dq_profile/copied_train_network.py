...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        self._dq_profile_execution_log_handler = None
        execution_log_path = getattr(args, "dq_profile_execution_log_path", None)
        if execution_log_path:
            execution_handler = logging.FileHandler(execution_log_path, mode="w", encoding="utf-8")
            execution_handler.setFormatter(logging.Formatter("%(asctime)s %(levelname)s %(name)s: %(message)s"))
            logging.getLogger().addHandler(execution_handler)
            self._dq_profile_execution_log_handler = execution_handler
        logger.info(
        ...
        if dq_begin_after_lr_warmup:
            from dq_profile.protocol import calculate_dq_begin_step

            dq_delta_begin_step = calculate_dq_begin_step(
        ...
        progress_bar = tqdm(
            range(args.max_train_steps - initial_step), smoothing=0, disable=(not accelerator.is_local_main_process or bool(getattr(args, "dq_profile_disable_progress_bar", False))), desc="steps"
        )
        ...
        for epoch in range(epoch_to_start, num_train_epochs):
            ...
            epoch_dataloader = skipped_dataloader or train_dataloader
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    and global_step == dq_delta_begin_step
                ):
                    return dq_profile_runtime.run_from_boundary(
