...
def _fake_quantize_levels_with_q(
    ...
    *,
    ...
    if mode == "det":
        ...
        elif rand.shape != probs.shape:
            raise ValueError(f"rand shape {tuple(rand.shape)} does not match x shape {tuple(probs.shape)}")
        q = torch.floor(q_clamp) + (rand.to(device=x.device, dtype=torch.float32) < probs).to(q_clamp.dtype)
    else:
...
class LoRAModule(torch.nn.Module):
    ...
    def __init__(
        ...
        self.dq_profile_context: Optional[ProfileQuantContext] = None
    ...
    def _record_dq_stats_for_quantized(
        self,
        x_in: torch.Tensor,
        quantized: torch.Tensor,
        scale: torch.Tensor,
        ...
        self._record_dq_stats(x_in, quantized, q_clamp, s, qmax)

    def _can_use_fused_triton_stats(self) -> bool:
        mgr = self.dq_stats_manager
        return bool(
            mgr is not None
            and mgr.active
            ...
            and triton_fake_quantize_levels_stoch_with_stats is not None
        )

    def _fake_quantize_levels_with_fused_stats(
        self,
        x_in: torch.Tensor,
        *,
        scale: torch.Tensor,
    ...
    def forward(self, x):
        ...
        profile_context = self.dq_profile_context
        if profile_context is not None:
            profile_context.record_module_invocation(self.lora_name)

        def profile_rand(tensor: torch.Tensor) -> Optional[torch.Tensor]:
            if profile_context is None or profile_context.mode != "candidate" or self.delta_q_mode != "stoch":
                return None
            invocation = profile_context.claim_invocation(self.lora_name)
            return profile_context.rand_for(tensor, module_name=self.lora_name, invocation=invocation)

        dropout_enabled = profile_context is None or profile_context.dropout_enabled

        # module dropout
        if self.module_dropout is not None and self.training and dropout_enabled:
            module_dropout_draw = torch.rand(1)
            module_dropped = bool(module_dropout_draw < self.module_dropout)
            if profile_context is not None:
                profile_context.record_dropout_site(
                    module_name=self.lora_name,
                    kind="module_dropout",
                    probability=float(self.module_dropout),
                    shape=tuple(module_dropout_draw.shape),
                    actual=module_dropout_draw,
                )
            if module_dropped:
        ...
        # normal dropout
        if self.dropout is not None and self.training and dropout_enabled:
            if profile_context is not None:
                # F.dropout keeps its production implementation.  The site
                # contract plus before/after global RNG fingerprints identifies
                # its generated network-dropout mask without persisting it.
                profile_context.record_dropout_site(
                    module_name=self.lora_name,
                    kind="network_dropout",
                    probability=float(self.dropout),
                    shape=tuple(lx.shape),
                )
            lx = torch.nn.functional.dropout(lx, p=self.dropout)

        # rank dropout
        if self.rank_dropout is not None and self.training and dropout_enabled:
        ...
        if (
            ...
            and profile_context.mode == "shadow"
            and self.delta_q_enabled
            and self.delta_q_on_z
            and self.delta_q_bits is not None
            and self.delta_q_bits > 0
        ):
            lx = profile_context.attach_shadow_hook(
                lx,
                module_name=self.lora_name,
                target="z",
                bits=self.delta_q_bits,
                granularity=self.delta_q_granularity,
                stat=self.delta_q_stat,
                quant_mode=self.delta_q_mode,
                use_triton_scale=self.delta_q_use_triton,
            )
        ...
        if (
            ...
            and self.delta_q_enabled
            and self.delta_q_on_z
            and (profile_context is None or profile_context.mode != "shadow")
        ):
            if self.delta_q_bits is not None and self.delta_q_bits > 0:
                qmax = (1 << (self.delta_q_bits - 1)) - 1
                x_in = lx
                rand_t = profile_rand(x_in)
                if (
                    profile_context is not None
                    and profile_context.mode == "candidate"
                    and profile_context.mechanism != "full"
                ):
                    with torch.no_grad():
                    ...
                    q_value, q_clamp, scale_t = _fake_quantize_levels_with_q(
                        x_in, scale=z_scale, qmin=-qmax, qmax=qmax, mode=self.delta_q_mode, rand=rand_t
                    )
                    x_clamped = (q_clamp * scale_t).to(x_in.dtype)
                    lx = profile_context.apply_mechanism(x_in, q_value, x_clamped)
                ...
                elif self.dq_stats_manager is not None and self.dq_stats_manager.wants_scope(self.dq_scope) and self.dq_stats_manager.active:
                    with torch.no_grad():
                        z_scale = compute_scale_bits(
                            lx,
                            bits=self.delta_q_bits,
        ...
        if (
            ...
            delta = profile_context.attach_shadow_hook(
                delta,
                module_name=self.lora_name,
                target="delta",
                bits=self.delta_q_bits,
        ...
        if (
            ...
            if self.delta_q_bits is not None and self.delta_q_bits > 0:
                ...
                if (
                    ...
                    q_value, q_clamp, scale_t = _fake_quantize_levels_with_q(
                        x_in, scale=d_scale, qmin=-qmax, qmax=qmax, mode=self.delta_q_mode, rand=rand_t
                    )
                    x_clamped = (q_clamp * scale_t).to(x_in.dtype)
                    delta = profile_context.apply_mechanism(x_in, q_value, x_clamped)
                    if self.dq_stats_manager is not None and self.dq_stats_manager.wants_scope(self.dq_scope) and self.dq_stats_manager.active:
...
class LoRANetwork(torch.nn.Module):
    ...
    def set_dq_profile_context(self, context: Optional[ProfileQuantContext]):
        for lora in self.text_encoder_loras + self.unet_loras:
            lora.dq_profile_context = context
